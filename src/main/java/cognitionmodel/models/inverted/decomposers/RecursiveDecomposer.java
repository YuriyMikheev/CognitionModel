...
public class RecursiveDecomposer<T extends Agent> implements Decomposer<Agent> {
    ...
    public RecursiveDecomposer(InvertedIndex invertedIndex, String predictingField, boolean modelcashed) {
        this(invertedIndex, predictingField, modelcashed, 3, null, false);

    }

    public RecursiveDecomposer(InvertedIndex invertedIndex, String predictingField, boolean modelcashed, int maxDepth, Function<Agent, Boolean> agentFilter){
        this(invertedIndex, predictingField, modelcashed, maxDepth, agentFilter, false);
    }
    public RecursiveDecomposer(InvertedIndex invertedIndex, String predictingField, boolean modelcashed, int maxDepth, Function<Agent, Boolean> agentFilter, boolean mustHavePredictingField){
    ...
    private Iterator<Point> getAgentFieldsIterator(Agent agent, Tuple record){
        ...
        return new Iterator<Point>() {
            ...
            int start = ag.getFields4view().isEmpty()? -1: ag.getFields().stream().max().getAsInt(), fi = start, vi = -1;
