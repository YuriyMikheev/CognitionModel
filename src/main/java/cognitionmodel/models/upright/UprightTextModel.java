...
public class UprightTextModel {
    ...
    public String generate(String text, int attentionSize){
        ...
        UprightTextComposer composer = new UprightTextComposer(in.size(), in.size());
    ...
    public List<UrAgent> makeAgentsListWrong(List<Integer> in, int attentionSize){
        ...
        for (Iterator<Integer> iterator = dataSet.getTextTokens().iterator(); ti < getDataSet().getTextTokens().size() + attentionSize; ti++ ) {
            ...
            if (!anypoints.isEmpty())
                if (ti - anypoints.getFirst().position >= attentionSize) {// || (ti - anypoints.getFirst().position < attentionSize && dataSet.getTextTokens().size() - ti < attentionSize)) {
                    UrPoint p1 = anypoints.pop();
                    RoaringBitmap pos = idx.get(p1.token);

                    for (int i1 : pos) {
                        if (p1.position < dataSet.getTextTokens().size()) {
                            points.add(new UrPoint(i1, p1.token));

                            for (Iterator<UrPoint> pointIterator = anypoints.iterator(); pointIterator.hasNext(); ) {
                                UrPoint p2 = pointIterator.next();
                                if (p2.position - p1.position < attentionSize && p2.position < dataSet.getTextTokens().size()) {
                                    RoaringBitmap pos2 = idx.get(p2.token);
                                    for (int i2 : pos2)
                                        if (abs(i2 - i1) < attentionSize & i2 > i1 && abs(p1.position - p2.position) == abs(i1 - i2))
                                            points.add(new UrPoint(i2, p2.token));
                                }
                            }
                            if (!points.isEmpty()) {
