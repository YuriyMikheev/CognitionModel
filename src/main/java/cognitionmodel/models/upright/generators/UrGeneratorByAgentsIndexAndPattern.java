...
public class UrGeneratorByAgentsIndexAndPattern implements UrGeneratorInterface {
    ...
    public UrGeneratorByAgentsIndexAndPattern(@NotNull TextIndex textIndex, UrTextDataSet dataSet, int[] relationTypes) throws IOException {
    ...
    public List<UrAgent> newAgents(@NotNull List<UrAgent> inAgents, int attentionSize, int variants, int[] newPointsPositions){
        ...
        AtomicLong nn = new AtomicLong();
        nn.set(0);

        ArrayList<Pattern> patterns = new ArrayList<>(new FullGridIterativePatterns(newPointsPositions.length, attentionSize, 10).getPatterns().stream().filter(pattern -> pattern.getBitSet().cardinality()>1).toList());
        ...
        for (Iterator<UrAgent> iterator = in.listIterator(); iterator.hasNext(); ) {
            ...
            cfl.add(CompletableFuture.supplyAsync(() -> {
                ...
                for (BatchedIterator gIterator = new BatchedIterator(agent.getIdx()); gIterator.hasNext(); ) {
                    ...
                    long idx;
                    try {
                        while (gIterator.hasNext() && (idx = gIterator.next()) != -1) {
